...
class PostgresStore:
    ...
    async def connect(self) -> None:
        ...
        # Try to create pgroonga index (optional, may fail)
        try:
            async with self._pool.acquire() as conn:
                await conn.execute(
                    """
                    CREATE INDEX IF NOT EXISTS idx_memories_content_pgroonga
                    ON memories USING pgroonga (content)
                    """
                )
            self._pgroonga_available = True
            logger.info("pgroonga full-text index created")
        except Exception as e:
            self._pgroonga_available = False
            logger.warning("pgroonga index not available: %s", e)
    ...
    async def save_episode(self, episode: Episode) -> Episode:
        ...
        embeddings = provider.embed([episode.summary])
        embedding_vec = embeddings[0]
